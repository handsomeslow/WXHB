...
public class DaigouGroupPresenter implements DaigouGroupContract.Presenter {
    List<LuckyGroupInfo> infoList;
    private DaigouGroupContract.View view;

    public DaigouGroupPresenter(DaigouGroupContract.View view) {
        this.view = view;
    ...
    public void pullGroupList() {
        AVQuery<AVObject> groupQuery = new AVQuery<>(CloudContentUtil.DAIGOU_GROUP_TABLE);
        groupQuery.include(CloudContentUtil.DAIGOU_GROUP_OWNER);
        ...
        groupQuery.findInBackground(new FindCallback<AVObject>() {
            ...
            public void done(List<AVObject> list, AVException e) {
                ...
                for (AVObject avObject:list){
                    ...
                    info.setContent(avObject.getString(CloudContentUtil.DAIGOU_GROUP_CONTENT));
                    info.setOwer(avObject.getAVUser(CloudContentUtil.DAIGOU_GROUP_OWNER));
                    info.setPublishDate(avObject.getCreatedAt());
                    info.setPhotoList(avObject.getList(CloudContentUtil.DAIGOU_GROUP_PHOTOS));
                    infoList.add(info);
    ...
    public void addComment(final String content, final String id, final int position) {
        final AVObject group = AVObject.createWithoutData(CloudContentUtil.DAIGOU_GROUP_TABLE, id);
